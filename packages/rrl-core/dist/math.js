 ...
 */
export function applyRounding(value, mode = 'BANKERS') {
 ...
 */
export function weeklyRate(apy) {
 ...
 */
export function calculateWeeklyDeposit(rigorLevel, year, customAmount) {
    if (rigorLevel === 'light') {
        if (year <= 1)
            return 100 / 4.33; // $100/month
        else if (year <= 2)
            return 150 / 4.33; // $150/month
        else if (year <= 3)
            return 200 / 4.33; // $200/month
        else
            return 250 / 4.33; // $250/month
    }
 ...
 */
export function calculateWeeklyDCA(rigorLevel, strandBalance) {
    const dcaPercent = 0.1; // 10% of Strand 1 balance
    if (rigorLevel === 'light')
        return Math.min(strandBalance * dcaPercent, 1000);
    else if (rigorLevel === 'medium')
        return Math.min(strandBalance * dcaPercent, 5000);
    else if (rigorLevel === 'heavy')
        return Math.min(strandBalance * dcaPercent, 10000);
    else
        return Math.min(strandBalance * dcaPercent, 2000); // Custom
}
